...
;---------------------
;>1 list
;  list_get_from_end - return last list entry
; INPUTS
...
;move pointer back to previous entry
  ...
  jmp	short update_put_ptr
at_top:
;special kludge, save last entry adr for list_get_from_end, it needs
...
update_put_ptr:
  mov	[edx+list.list_tail_ptr],esi
  xor	eax,eax			;set success flag
list_get_from_end_exit:
  or	eax,eax
  ret
